...
class JoystickController(object):
    ...
    def set_throttle_reversed(self, axis_val):
        #this value is often reversed, with positive value when pulling down
        from numpy import negative
        self.last_throttle_axis_val = negative(axis_val)
        self.throttle = (negative(self.throttle_dir) * axis_val * self.throttle_scale)
        #print("throttle", self.throttle)
        self.on_throttle_changes()
...
class XboxOneController(Joystick):
    #An interface to a physical joystick available at /dev/input/js0
    def __init__(self, *args, **kwargs):
        super(XboxOneController, self).__init__(*args, **kwargs)
...
class XboxOneJoystickController(JoystickController):
    ...
    def init_js(self):
        ...
        '''
        try:
            self.js = XboxOneController(self.dev_fn)
            self.js.init()
        except FileNotFoundError:
            print(self.dev_fn, "not found.")
            self.js = None
    ...
    def init_trigger_maps(self):
        ...
        self.button_down_trigger_map = {
            ...
            'Select': self.toggle_constant_throttle,
        }

        self.axis_trigger_map = {
            'LSH': self.set_steering,
            'RSV': self.set_throttle,
            # Forza Mode
            'RT': self.set_throttle_reversed,
            'LT': self.set_throttle,
        }

...
class KerasLinear(KerasPilot):
    ...
    def run(self, img_arr):
        #from donkeycar.parts.orimodel import processImage
    ...
    def combineThresholds(self, img, s_thresh=(115, 255), sx_thresh=(100, 150)):
        import cv2
        img = np.copy(img)
        ...
        l_channel = hls[:,:,1]
        s_channel = hls[:,:,2]
        s_channel = cv2.normalize(s_channel,None,0,255,cv2.NORM_MINMAX,cv2.CV_8U)
    ...
    def processImage(self, img): 
        ...
        warped = self.warpImage(img)
        combinedThreshold = self.combineThresholds(warped)
        rgb = Image.fromarray(combinedThreshold*255)
        rgb = rgb.convert('RGB')
        one_byte_scale = 1.0 / 255.0
        return np.array(rgb).astype(np.float32) * one_byte_scale
...
def default_n_linear(num_outputs, input_shape=(180, 320, 3), roi_crop=(0, 0)):
...
class KerasRNN_LSTM(KerasPilot):
    def __init__(self, image_w =320, image_h=180, image_d=3, seq_length=3, num_outputs=2, *args, **kwargs):
    ...
    def combineThresholds(self, img, s_thresh=(170, 255), sx_thresh=(100, 150)):
        import cv2
        img = np.copy(img)
        # Convert to HLS color space and separate the V channel
        hls = cv2.cvtColor(img, cv2.COLOR_RGB2HLS)
